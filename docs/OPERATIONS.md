...
| `quote.py` | aagrapevine.org `/`, aalavina.org `/` (`sources.<pub>.quote_page`) → `quote.json` (items + 14-day `history`) | The `#quote-of-the-day` teaser (`article.node--type-quote`): heading date (year inferred around today, Central), quote text (outer quotation marks cleaned, never translated), attribution / source split at "From:" / "De", the publication's own e-mail sign-up link. One request per site, in the full AND the quick run | Falls back to the view embed near the top of the page; a publication that fails keeps its previous quote (and last known sign-up link), `ok=false`; an older quote than the one known never replaces it | `--dry-run`, `--only gv\|lv`, `--html-dir DIR`, `--save-html DIR` |
