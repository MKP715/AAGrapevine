...
Run:  python -m pytest -q tests/test_send_digest.py
"""
...
def site_dir(tmp_path, monkeypatch):
    ...
    for name in ("whatsnew", "events", "announcements"):
        (d / f"{name}.json").write_text(json.dumps({"items": []}), encoding="utf-8")
    monkeypatch.setattr(D, "SITE_DIR", d)
...
def test_botm_teaser_in_both_languages(site_dir, tmp_path):
    write_shop(site_dir, [offer("gv"), offer("lv")])
    html, text = preview(tmp_path)
    en, es = halves(text)
...
def test_ended_offer_is_left_out(site_dir, tmp_path):
    write_shop(site_dir, [offer("gv", ends="2026-09-27"), offer("lv", discount_pct=15, sale_price=12.74)])
    html, text = preview(tmp_path)          # as of Sep 28: the GV offer ended the day before
...
def test_different_percents_use_the_plain_heading(site_dir, tmp_path):
    write_shop(site_dir, [offer("gv"), offer("lv", discount_pct=15, sale_price=12.74)])
    _, text = preview(tmp_path)
    en, es = halves(text)
    assert "BOOK OF THE MONTH\n-----------------\n" in en and "% OFF" not in en
    assert "LIBRO DEL MES\n-------------\n" in es


def test_no_shop_data_keeps_the_toolkit_line(site_dir, tmp_path):
    html, text = preview(tmp_path)          # no shop.json at all
    ...
    (site_dir / "shop.json").write_text("{not json", encoding="utf-8")   # a broken file never stops the digest
    _, text = preview(tmp_path)
    assert "BOOK OF THE MONTH" not in text and "/monthly/2026-09/" in text


def test_botm_alone_is_not_news(site_dir):
    write_shop(site_dir, [offer("gv"), offer("lv")])
...
def test_month_is_counted_in_central_time(site_dir):
